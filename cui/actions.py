...
def playout(state):
    ...
    #ゲームが終わっていない場合
    # -(次の手番のプレイヤーの得る報酬)
    return -playout(state.next(Random(state)))
...
def UCT(state):
    ...
    class node:
        ...
        def expand(self):   #子ノードを列挙
            self.child_nodes = tuple(node(self.state.next(action)) \
...
def User(state):
    print(state)
    actions = state.legal_actions
    state.print_legal_actions()
    choice = int(input("Which action number?\t"))
    if choice not in [i for i in range(len(actions))]:
        print("##input error##")
        User(state)
